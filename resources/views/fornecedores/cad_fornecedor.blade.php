            ...
            <div class="col-md-12">
                ...
                <ul class="nav md-pills pills-info mb-4" id="tabs">
                    <li class="nav-item pl-0">
                        <a class="nav-link active " data-toggle="tab" href="#panelGrid" role="tab">Fornecedores</a>
                    </li>
                    <li class="nav-item">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#pessoa").select2({
                ...
                ajax: {
                    type: 'POST',
                    dataType: "json",
                    url: '{{route('fornecedor.get_pessoa')}}',
        ...
        datatable = $('#table-fornecedor').customGrid({
            "columns": [
                {
                    data: 'id',
                    name: 'id',
    ...
    function openModalConfirmDelete(fornecedor_id){
        id_fornecedor = fornecedor_id;
        $('#modalConfirmDelete').modal('show');
    }


    function Editar(fornecedor_id) {
        let row = datatable.row(fornecedor_id - 1).data();
    ...
    function Excluir(fornecedor_id) {
        $.ajax({
            type: 'POST',
            url: '{{route('fornecedor.delete')}}' + '/' + fornecedor_id,
    ...
    function getDataGrid() {
        $.ajax({
            type: 'POST',
            url: '{{route('fornecedor.data_grid')}}',
            success: function (resposta) {
    ...
    function Salvar() {
        $.ajax({
            type: 'POST',
            url: id_fornecedor > 0 ? "{{route('fornecedor.update')}}" + '/' + id_fornecedor : "{{route('fornecedor.insert')}}",
            data: {
                pessoa:$("#pessoa").val(),
